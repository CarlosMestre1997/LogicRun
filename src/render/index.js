...
export function drawLevel(ctx, level, state) {
  const gridInfo = drawLevelFrame(ctx, level, state);
...
function drawLevelFrame(ctx, level, state) {
  ...
  if (!state.ghostVisible) {
    drawStartie(ctx, state, gridInfo);
  }
  
  // Draw ghost if visible
  if (state.ghostVisible && state.ghostY !== undefined && gridInfo) {
    drawGhostAtPosition(ctx, state, gridInfo);
  }
